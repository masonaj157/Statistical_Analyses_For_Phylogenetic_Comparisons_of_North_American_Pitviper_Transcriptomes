...
setwd('~/Dropbox/Andrew/DataSets_Manuscripts/Crotalus_reanalysis/project_github/Statistical_Analyses_For_Phylogenetic_Comparisons_of_North_American_Pitviper_Transcriptomes')
...
Average_per_species = function( toxin_counts_object, species ){
  ...
  toxins_sd<-sd(tmp$Toxin_count)
  ave_output<-tibble(species,ave_toxins,toxins_sd)
  return(ave_output)
}
...
calculate_clrTPM = function(RSEM,Sample){
  ...
  clr=log((tmp$TPM / gm))
  clr_min<-min(clr)
  #rep_geo_mean<-rep(gm,length(clr))
  tmp<-cbind(tmp,clr)
  #tmp<-cbind(tmp,clr,rep_geo_mean)
  return(tmp)
}
...
## Calculate expression evenness of families
...
calculate_total_expression_family = function(RSEM_summary,tox_fam,Sample){
  ### subset list by sample
  tmp_sample<-subset(RSEM_summary,RSEM_summary$ID==Sample)
  tmp<-subset(tmp_sample,grepl(tox_fam,tmp_sample$Transcript)==TRUE)
...
Summarize_expression_by_family_expression = function(RSEM_summary,tox_fam){
  Family_table=data.frame(ID=factor(),Species=factor(), Toxin_family=factor(),Tox_count=numeric(),                            sum_TPM=numeric(),mean_TPM=numeric(),sd_TPM=numeric(),sum_clrTPM=numeric(),
                      mean_clrTPM=numeric(),sd_clrTPM=numeric(), min_clrTPM=numeric(), sum_perc=numeric(), mean_perc=numeric(), sd_perc=numeric())
...
## rsem data
RSEM<-read.csv("~/Dropbox/Andrew/DataSets_Manuscripts/Crotalus_reanalysis/project_github/Statistical_Analyses_For_Phylogenetic_Comparisons_of_North_American_Pitviper_Transcriptomes/input_data/6.1_Final_RSEM_lineage.csv")
## Drop 0 values
RSEM<-subset(RSEM,RSEM$TPM > 0)

clustered_Toxin_counts<-read.csv("~/Dropbox/Andrew/DataSets_Manuscripts/Crotalus_reanalysis/project_github/Statistical_Analyses_For_Phylogenetic_Comparisons_of_North_American_Pitviper_Transcriptomes/input_data/5.4_Final_indiv_toxincount_clustered_lineages.csv")
clustered_Toxin_counts<-clustered_Toxin_counts[-c(73),]

Toxin_kmers<-read.table("~/Dropbox/Andrew/DataSets_Manuscripts/Crotalus_reanalysis/project_github/Statistical_Analyses_For_Phylogenetic_Comparisons_of_North_American_Pitviper_Transcriptomes/input_data/7.1_toxin_kmers_clustered_lineage.txt", quote="\"", comment.char="")
colnames(Toxin_kmers) = c('Sample','Species','kmer','count')

Reads<-read.csv("~/Dropbox/Andrew/DataSets_Manuscripts/Crotalus_reanalysis/project_github/Statistical_Analyses_For_Phylogenetic_Comparisons_of_North_American_Pitviper_Transcriptomes/input_data/0.5_Read_Counts.csv")
Reads<-Reads[,1:3]
Reads$Reads<-as.integer(as.character(Reads$Reads))
#colnames(Toxin_kmers) = c('Sample','Species','kmer','count')
...
# Drop some rows of hybrid individuals from the dataset 
...
Collect info for assessing technical variation.
```{r}
Kmer_diversity<-as.data.frame(Kmer_diversity)
species_idx = match(clustered_Toxin_counts[,1],Kmer_diversity[,1])
Kmer_diversity_reordered<-Kmer_diversity[species_idx,]

RSEM_sample_evenness<-as.data.frame(RSEM_sample_evenness)
species_idx_2 =  match(clustered_Toxin_counts[,1],as.data.frame(RSEM_sample_evenness)[,1])
RSEM_sample_evenness_reordered<-RSEM_sample_evenness[species_idx_2,]

Reads<-as.data.frame(Reads)
species_idx_3 = match(clustered_Toxin_counts[,1],Reads[,1])
Reads_reordered<-Reads[species_idx_3,]

```


Need to fix the toxin groups because there are some samples without certain toxins
```{r}

PLA2_expression_sample_fixed<-as.data.frame(PLA2_expression_sample)
for (i in 1:nrow(clustered_Toxin_counts)){
  if (clustered_Toxin_counts$Sample[i] %in% PLA2_expression_sample$Sample){next}
  else{
    entry<-c(as.character(clustered_Toxin_counts$Sample[i]),as.character(clustered_Toxin_counts$Species[i]),"PLA2",as.numeric(0),
      as.numeric(0),as.numeric(0),as.numeric(0),NA,NA,NA,as.numeric(0),as.numeric(0),as.numeric(0))
  }
  PLA2_expression_sample_fixed<-rbind(PLA2_expression_sample_fixed,entry)
}


CTL_expression_sample_fixed<-as.data.frame(CTL_expression_sample)
for (i in 1:nrow(clustered_Toxin_counts)){
  if (clustered_Toxin_counts$Sample[i] %in% CTL_expression_sample$Sample){next}
  else{
    entry<-c(as.character(clustered_Toxin_counts$Sample[i]),as.character(clustered_Toxin_counts$Species[i]),"CTL",as.numeric(0),
      as.numeric(0),as.numeric(0),as.numeric(0),NA,NA,NA,as.numeric(0),as.numeric(0),as.numeric(0))
  }
  CTL_expression_sample_fixed<-rbind(CTL_expression_sample_fixed,entry)
}

SVMP_expression_sample_fixed<-as.data.frame(SVMP_expression_sample)
for (i in 1:nrow(clustered_Toxin_counts)){
  if (clustered_Toxin_counts$Sample[i] %in% SVMP_expression_sample$Sample){next}
  else{
    entry<-c(as.character(clustered_Toxin_counts$Sample[i]),as.character(clustered_Toxin_counts$Species[i]),"SVMP",as.numeric(0),
      as.numeric(0),as.numeric(0),as.numeric(0),NA,NA,NA,as.numeric(0),as.numeric(0),as.numeric(0))
  }
 SVMP_expression_sample_fixed<-rbind(SVMP_expression_sample_fixed,entry)
}

SVSP_expression_sample_fixed<-as.data.frame(SVSP_expression_sample)
for (i in 1:nrow(clustered_Toxin_counts)){
  if (clustered_Toxin_counts$Sample[i] %in% SVSP_expression_sample$Sample){next}
  else{
    entry<-c(as.character(clustered_Toxin_counts$Sample[i]),as.character(clustered_Toxin_counts$Species[i]),"SVSP",as.numeric(0),
      as.numeric(0),as.numeric(0),as.numeric(0),NA,NA,NA,as.numeric(0),as.numeric(0),as.numeric(0))
  }
  SVSP_expression_sample_fixed<-rbind(SVSP_expression_sample_fixed,entry)
}



species_idx_4 = match(clustered_Toxin_counts[,1],PLA2_expression_sample_fixed[,1])
PLA2_expression_sample_reordered<-PLA2_expression_sample_fixed[species_idx_4,]

species_idx_5 = match(clustered_Toxin_counts[,1],SVMP_expression_sample_fixed[,1])
SVMP_expression_sample_reordered<-SVMP_expression_sample_fixed[species_idx_5,]

species_idx_6 = match(clustered_Toxin_counts[,1],CTL_expression_sample_fixed[,1])
CTL_expression_sample_reordered<-CTL_expression_sample_fixed[species_idx_6,]

species_idx_7 = match(clustered_Toxin_counts[,1],SVSP_expression_sample_fixed[,1])
SVSP_expression_sample_reordered<-SVSP_expression_sample_fixed[species_idx_7,]



Indiv_Venom_data<-cbind(clustered_Toxin_counts, Kmer_diversity_reordered[,3], RSEM_sample_evenness_reordered[,3],
                        PLA2_expression_sample_reordered$Tox_numbers,PLA2_expression_sample_reordered$mean_clrTPM,
                        SVMP_expression_sample_reordered$Tox_numbers,SVMP_expression_sample_reordered$mean_clrTPM,
                        CTL_expression_sample_reordered$Tox_numbers,CTL_expression_sample_reordered$mean_clrTPM,
                        SVSP_expression_sample_reordered$Tox_numbers,SVSP_expression_sample_reordered$mean_clrTPM)


## Also need to add MPD values to the individual table. These will be the species values repeated for each indivudal

##First make a hash table where species are keys and MPD are values
library(hash)
species_mpd_hash<-hash(MPD_Venom_data$Species, MPD_Venom_data$DB)
DB_values<-c()
for (i in 1:nrow(Indiv_Venom_data)){
  if (Indiv_Venom_data[i,2] %in% keys(species_mpd_hash)){
  entry<-values(species_mpd_hash,Indiv_Venom_data[i,2])[[1]]
  DB_values<-c(DB_values,entry)
  } else{DB_values<-c(DB_values,NA)}
}

samples_transcript_num<-tibble(indiv=factor(), n_transcripts=integer())
for (sample in unique(RSEM$ID)){
  indiv_n_transcripts<-nrow(subset(RSEM,RSEM$ID == sample))
  row<-tibble(indiv=sample, n_transcripts=indiv_n_transcripts)
  samples_transcript_num<-rbind(samples_transcript_num,row)
}
samples_transcript_num<-as.data.frame(samples_transcript_num)

species_idx_8 = match(clustered_Toxin_counts[,1],samples_transcript_num[,1])
samples_transcript_num<-samples_transcript_num[species_idx_8,]

Indiv_Venom_data<-cbind(Indiv_Venom_data,samples_transcript_num$n_transcripts,Reads_reordered$Reads)


Indiv_Venom_data<-cbind(Indiv_Venom_data,DB_values)
colnames(Indiv_Venom_data) =c('Sample','Species','GC','SD','ED','PLA2_num','PLA2_mean','SVMP_num','SVMP_mean','CTL_num','CTL_mean','SVSP_num','SVSP_mean','transcript_n','reads','DB')

## Drop hybrids from the Individual dataset
Indiv_Venom_data<-Indiv_Venom_data[-c(6,13,14,15,106),]

Indiv_Venom_MPD<-subset(Indiv_Venom_data,is.na(Indiv_Venom_data$DB) == FALSE)
...
Let's perform some very simple tests of the roles of assembly completeness (here represented by number of transcripts) and within species variation
```{r}
library("car")
library(rr2)

## First test if our explanatory variables vary according to assembly completeness
GC_reads_lm<-lm(Indiv_Venom_data$GC~Indiv_Venom_data$reads)
plot(Indiv_Venom_data$GC~Indiv_Venom_data$reads, pch=21,bg='gray',cex=1.5,
     xlab='Number Merged Reads',ylab="Number Toxin Transcripts")
abline(a=GC_reads_lm$coefficients[[1]],b=GC_reads_lm$coefficients[[2]],lty=2,lwd=3)
summary(GC_reads_lm)
Anova(lm(Indiv_Venom_data$GC~Indiv_Venom_data$Species*Indiv_Venom_data$reads))


GC_trans_lm<-lm(Indiv_Venom_data$GC~Indiv_Venom_data$transcript_n)
plot(Indiv_Venom_data$GC~Indiv_Venom_data$transcript_n, pch=21,bg='gray',cex=1.5,
     xlab='Number Expressed Transcripts',ylab="Number Toxin Transcripts")
abline(a=GC_trans_lm$coefficients[[1]],b=GC_trans_lm$coefficients[[2]], lty=2,lwd=3)
summary(GC_trans_lm)
Anova(lm(Indiv_Venom_data$GC~Indiv_Venom_data$Species*Indiv_Venom_data$transcript_n))
...
Assess within species variation
```{r}
library("cowplot")
library("ggpubr")


Venom_data[order(Venom_data$Sample_number),]

ggboxplot(Indiv_Venom_data, x="Species", y="GC", fill="Species", add = "dotplot", add.params=list(size=0.6),legend="none",x.text.angle=90, order = Venom_data[order(Venom_data$Sample_number),]$Species,xlab="Lineage")+font("x.text", size = 10)


ggboxplot(Indiv_Venom_data, x="Species", y="SD", fill="Species", add = "dotplot", add.params=list(size=0.6),legend="none",x.text.angle=90, order = Venom_data[order(Venom_data$Sample_number),]$Species,xlab="Lineage")+font("x.text", size = 10)



ggboxplot(Indiv_Venom_data, x="Species", y="ED", fill="Species", add = "dotplot", add.params=list(size=0.6),legend="none",x.text.angle=90, order = Venom_data[order(Venom_data$Sample_number),]$Species,xlab="Lineage")+font("x.text", size = 10)
...
diet_best_model_lam <- best(diet_result_lam, boot=500)
diet_average_model_lam <- average(diet_result_lam,avg_method = "conditional", boot=500)

## because model 1 was within 2 CICc
diet_model1<-choice(diet_result_lam,"Model1", boot=500)
...
p4<-coef_plot(diet_best_model_lam, error_bar = 'ci',order_by="causal")
p5<-coef_plot(diet_average_model_lam,error_bar = 'ci',order_by="causal")
p6<-plot(diet_model1, curvature = 0.05)
p7<-coef_plot(diet_model1,error_bar = 'ci',order_by="causal")
...
Plots of the relationship between proximal estimates of sequence diversity and expression diversity against diet breadth.
```{r}
library(phylolm)
library(rr2)
...
DB_SD<-phylolm(DB~SD, MPD_Venom_data, pruned.tree, model = c("lambda"))
summary(DB_SD)
R2(DB_SD,phy=pruned.tree)
plot(MPD_Venom_data$DB~MPD_Venom_data$SD,pch=21,bg='darkgray',cex=2)
abline(a=DB_SD$coefficients[[1]],b=DB_SD$coefficients[[2]], lty=2, lwd=4)


DB_ED<-phylolm(DB~ED, MPD_Venom_data, pruned.tree, model = c("lambda"))
summary(DB_ED)
R2(DB_ED,phy=pruned.tree)
plot(MPD_Venom_data$DB~MPD_Venom_data$ED,pch=21,bg='darkgray',cex=2)
abline(a=DB_ED$coefficients[[1]],b=DB_ED$coefficients[[2]], lty=2, lwd=4)
...
(SVSP_diet_best_model_lam2 <- choice(SVSP_diet_result_lam,"Model7"))
SVSP_diet_average_model_lam <- average(SVSP_diet_result_lam,avg_method = "conditional")

p16<-plot(SVSP_diet_s_lam)
p17<-plot(SVSP_diet_best_model_lam2,col=c("blue","green"), algorithm = 'mds')
...
DB_v_PLA2_mean_nodurissus<-phylolm(DB~PLA2_mean, MPD_Venom_data_sansdurissus, pruned.tree.one, model = c("lambda"))
summary(DB_v_PLA2_mean_nodurissus)
R2(DB_v_PLA2_mean_nodurissus,phy=pruned.tree.one)
...
abline(a=DB_v_PLA2_mean_nodurissus$coefficients[[1]],b=DB_v_PLA2_mean_nodurissus$coefficients[[2]], lty=2, lwd=4,col="red3")
...
DB_v_CTL_mean<-phylolm(DB~CTL_mean, MPD_Venom_data, pruned.tree.one, model = c("lambda"))
summary(DB_v_CTL_mean)
R2(DB_v_CTL_mean,phy=pruned.tree.one)
plot(MPD_Venom_data$DB~MPD_Venom_data$CTL_mean,pch=21,bg='yellow1',cex=2)
...
All_SVMPs_iqtree<-read.newick('/Users/andrewmason/Dropbox/Andrew/DataSets_Manuscripts/Crotalus_reanalysis/project_github/Statistical_Analyses_For_Phylogenetic_Comparisons_of_North_American_Pitviper_Transcriptomes/input_data/9.3_SVMP_alignments/All_SVMPs/All_SVMP_muscle_aln.faa.treefile')


SVMP_MPD_table<-Toxin_expression_MPD_TPM(SVMP_RSEM,All_SVMPs_iqtree)
#Drop hybrids
SVMP_MPD_table<-SVMP_MPD_table[-c(6,12,16,20,114),]
...
ggboxplot(SVMP_MPD_table, x="Species", y="ses.mpd.z", fill="deepskyblue", add = "dotplot", add.params=list(size=0.6),legend="none",x.text.angle=90, ylab = "standardized MGD SVMP", xlab = "Lineage", order=SVMP_MPD_summary[order(SVMP_MPD_summary$ses.mpd.z),]$species)+font("x.text", size = 10)
...
wilcox.test(SVMP_MPD_final$ses.mpd.z,mu=0, alternative = "two.sided")

SVMP_mode1<-subset(SVMP_MPD_final,SVMP_MPD_final$ses.mpd.z <= -1)
SVMP_mode2<-subset(SVMP_MPD_final,SVMP_MPD_final$ses.mpd.z > -1)
SVMP_mode2.1<-subset(SVMP_MPD_final,SVMP_MPD_final$ses.mpd.z > -1 & SVMP_MPD_final$ses.mpd.z < 0.7)
SVMP_mode2.2<-subset(SVMP_MPD_final,SVMP_MPD_final$ses.mpd.z > 0.7)
...
All_SVSPs_iqtree<-read.newick('/Users/andrewmason/Dropbox/Andrew/DataSets_Manuscripts/Crotalus_reanalysis/project_github/Statistical_Analyses_For_Phylogenetic_Comparisons_of_North_American_Pitviper_Transcriptomes/input_data/9.4_SVSP_alignments/All_SVSPs/All_SVSP_muscle_aln.faa.treefile')

SVSP_MPD_table<-Toxin_expression_MPD_TPM(SVSP_RSEM,All_SVSPs_iqtree)
SVSP_MPD_table<-SVSP_MPD_table[-c(6,12,16,20,115),]
...
ggboxplot(SVSP_MPD_table, x="Species", y="ses.mpd.z", fill="green3", add = "dotplot", add.params=list(size=0.6),legend="none",x.text.angle=90, ylab = "standardized MGD SVSP",xlab="Lineage",order=SVSP_MPD_summary[order(SVSP_MPD_summary$ses.mpd.z),]$species)+font("x.text")+font("x.text", size = 10)
...
wilcox.test(SVSP_MPD_final$ses.mpd.z,mu=0, alternative = "two.sided")


SVSP_mode1<-subset(SVSP_MPD_final,SVSP_MPD_final$ses.mpd.z <= 0.3)
SVSP_mode2<-subset(SVSP_MPD_final,SVSP_MPD_final$ses.mpd.z > 0.3)
...
p1<- ggtree(tree, aes(color=trait), ladderize = FALSE, continuous = TRUE, size=1.5) %<+% name_d + scale_color_gradientn(colours=topo.colors(n=40)) + geom_tiplab(cex=2.5,aes(label=paste(name)),col='black')
